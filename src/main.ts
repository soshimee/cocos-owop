...
class ClientPool {
	...
	public constructor() {
		OWOP.on(OWOP.events.tick, () => {
			...
			for (; task.index < 256; ++task.index) {
				...
				console.log(newCol, bgCol, blendedCol);
	...
	public get client() {
		for (const client of this.clients.values()) {
			...
			if (client.state === ClientState.Ready && client.bucket.value > client.bucket.rate / 3) return client;
		}
	}
	...
	public queueImage(canvas: HTMLCanvasElement, pos: Pos) {
		...
		const [chunkX, chunkY] = [Math.floor(pos.chunkX), Math.floor(pos.chunkY)];
...
export const main = () => {
	...
	const buildWindowConfTab = (container: HTMLDivElement) => {
		...
		container.append(sneakyLabel, btnsDiv);
	...
	const buildWindow = () => {
		const window = new TabbedWindow("cocos");
		window.getContainer().style.width = "300px";
		window.addTab("conn", "Connections");
		window.addTab("clip", "Clipboard");
		window.addTab("conf", "Options");
		window.setTab("conn");
		buildWindowConnTab(window.getTabContainer("conn")!);
		buildWindowClipTab(window.getTabContainer("clip")!);
		buildWindowConfTab(window.getTabContainer("conf")!);
	};
